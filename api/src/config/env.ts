...
export const config = {
  ...
  // Database
  MONGODB_URI:
    process.env['MONGODB_URI'] || 'mongodb://localhost:27017/projectaccel',
  MONGODB_TEST_URI:
...
export function validateConfig(): void {
  ...
  if (config.isProduction()) {
    ...
    if (usingDefaultSecrets) {
      ...
      console.error(
        'Please set strong, unique secrets for JWT_ACCESS_SECRET, JWT_REFRESH_SECRET, and SESSION_SECRET'
      );
      process.exit(1);
    }
  }
