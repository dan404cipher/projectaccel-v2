...
// Pre-save middleware to generate workspace ID
...
workspaceSchema.methods.addMember = function(userId: mongoose.Types.ObjectId, roleId: mongoose.Types.ObjectId, invitedBy?: mongoose.Types.ObjectId) {
  // Check if user is already a member
  const existingMember = this.members.find((member: IWorkspaceMember) => 
...
workspaceSchema.methods.removeMember = function(userId: mongoose.Types.ObjectId) {
  this.members = this.members.filter((member: IWorkspaceMember) => 
    member.userId.toString() !== userId.toString()
  );
  return this.save();
};
...
workspaceSchema.methods.updateMemberRole = function(userId: mongoose.Types.ObjectId, newRoleId: mongoose.Types.ObjectId) {
...
workspaceSchema.methods.getMember = function(userId: mongoose.Types.ObjectId): IWorkspaceMember | undefined {
  return this.members.find((member: IWorkspaceMember) => 
    member.userId.toString() === userId.toString()
  );
};
...
workspaceSchema.methods.isOwner = function(userId: mongoose.Types.ObjectId): boolean {
  return this.ownerId.toString() === userId.toString();
};

// Instance method to transfer ownership
workspaceSchema.methods.transferOwnership = function(newOwnerId: mongoose.Types.ObjectId) {
...
workspaceSchema.methods.getActiveMembersCount = function(): number {
  return this.members.filter((member: IWorkspaceMember) => member.status === 'active').length;
};

// Instance method to get members by status
workspaceSchema.methods.getMembersByStatus = function(status: 'active' | 'invited' | 'suspended'): IWorkspaceMember[] {
  return this.members.filter((member: IWorkspaceMember) => member.status === status);
};
