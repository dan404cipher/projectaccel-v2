...
const userSchema = new Schema<IUser>(
  {
    ...
    empId: {
      ...
      match: /^[A-Z]{2}\d{4}$/, // Format: VA1000, AB1001, etc.
    },
  },
  {
    timestamps: true,
...
userSchema.methods.addWorkspace = function (
  ...
  const existingWorkspace = this.workspaces.find(
    (ws: IUserWorkspace) => ws.workspaceId.toString() === workspaceId.toString()
  );
...
userSchema.methods.removeWorkspace = function (
  workspaceId: mongoose.Types.ObjectId
) {
...
userSchema.methods.updateWorkspaceRole = function (
  workspaceId: mongoose.Types.ObjectId,
...
userSchema.methods.getWorkspace = function (
  workspaceId: mongoose.Types.ObjectId
...
userSchema.methods.isInWorkspace = function (
  workspaceId: mongoose.Types.ObjectId
): boolean {
...
userSchema.methods.getWorkspaceProfile = function (
  workspaceId: mongoose.Types.ObjectId
) {
